...
if (!isMainModule({
	...
	const { installGaxiosFetchCompat } = await import("./gaxios-fetch-compat-CfCCbVkR.js");
...
function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	import("./root-help-pb4TK9ho.js").then(({ outputRootHelp }) => {
		outputRootHelp();
	}).catch(handleError);
	return true;
}
function runMainOrRootHelp(argv) {
	...
	import("./run-main-C58CblcF.js").then(({ runCli }) => runCli(argv)).catch((error) => {
