...
import { _ as resolveRunModelFallbacksOverride } from "./agent-scope-DoT9OqaV.js";
import "./defaults-CyhDhBAD.js";
import { s as isCliProvider } from "./model-selection-B6ao45a4.js";
import { C as parseNonNegativeByteSize, s as loadConfig } from "./io-cPs4dU7X.js";
...
import { o as normalizeAnyChannelId, s as normalizeChannelId } from "./registry-xyHjVLxh.js";
...
import { a as isInternalMessageChannel, d as resolveMessageChannel, o as isMarkdownCapableMessageChannel } from "./message-channel-DUrzQUcI.js";
import { M as resolveGroupSessionKey, l as updateSessionStore, n as loadSessionStore, u as updateSessionStoreEntry } from "./store-D_eNuCKK.js";
import "./runtime-C8dQugND.js";
import { s as getChannelPlugin } from "./plugins-DGdgUNcN.js";
import "./sessions-C8WesgCl.js";
import { a as resolveSessionTranscriptPath, i as resolveSessionFilePathOptions, r as resolveSessionFilePath } from "./paths-BPfguEtH.js";
...
import { a as stripHeartbeatToken, c as isSilentReplyPrefixText, l as isSilentReplyText, s as SILENT_REPLY_TOKEN } from "./heartbeat-FZaB85hC.js";
import { $c as resolveOriginAccountId, $l as applyReplyTagsToPayload, Bf as formatUsd, Ci as runCliAgent, Ff as buildFallbackNotice, Fy as resolveCronStyleNow, Ic as registerAgentRunContext, If as resolveFallbackTransition, Lc as enqueueFollowupRun, Nc as emitAgentEvent, Of as resolveBootstrapWarningSignaturesSeen, Pf as buildFallbackClearedNotice, Rf as estimateUsageCost, Si as setCliSessionId, V as resolveModelAuthMode, Vf as resolveModelCostConfig, Xc as createTypingSignaler, _y as isContextOverflowError, bg as hasNonzeroUsage, by as sanitizeUserFacingText, el as resolveOriginMessageProvider, eu as applyReplyThreading, gy as isCompactionFailureError, h as readPostCompactionContext, hl as resolveEffectiveBlockStreamingConfig, hu as runWithModelFallback, hy as BILLING_ERROR_USER_MESSAGE, lu as parseReplyDirectives, m as estimateMessagesTokens, nl as isReasoningTagProvider, nu as createReplyToModeFilterForChannel, ru as resolveReplyToMode, t as runEmbeddedPiAgent, tl as resolveOriginMessageTo, tu as isRenderablePayload, vg as derivePromptTokens, vy as isLikelyContextOverflowError, xg as normalizeUsage, xi as getCliSessionId, xy as isBillingErrorMessage, yf as resolvePathFromInput, yg as deriveSessionTotalTokens, yy as isTransientHttpError, zc as scheduleFollowupDrain, zf as formatTokenCount } from "./pi-embedded-D3aYWCrT.js";
...
import { P as resolveSandboxRuntimeStatus, a as ensureSandboxWorkspaceForSession } from "./sandbox-Dy3vKrxx.js";
import "./common-De-BVwIv.js";
import "./image-ops-BLXW6hKm.js";
import "./thinking-CSfcimRZ.js";
import "./path-alias-guards-CgP-6a1d.js";
import { a as resolveSandboxedMediaSource, t as assertMediaNotDataUrl } from "./sandbox-paths-Bk2ZwMsR.js";
import "./channel-actions-BzT1SAu9.js";
import { a as isAudioFileName } from "./mime-D7Q9o2pi.js";
...
import { p as resolveSendableOutboundReplyParts, s as hasOutboundReplyContent } from "./reply-payload-MXtGsVoh.js";
import "./hook-runtime-DEbp5SYL.js";
import { n as isDiagnosticsEnabled, t as emitDiagnosticEvent } from "./diagnostic-events-BHGnMB4E.js";
...
import { d as lookupCachedContextTokens, l as lookupContextTokens } from "./level-overrides-pDiOPsgd.js";
...
import { n as resolveCronStorePath, t as loadCronStore } from "./config-runtime-ST43sCMq.js";
...
import { n as createBlockReplyContentKey, r as createBlockReplyPipeline, t as createAudioAsVoiceBuffer } from "./block-reply-pipeline-D-ThaDQw.js";
import { n as incrementCompactionCount } from "./session-updates-0niHa6CW.js";
...
*/
function computeContextHash(messages) {
	const tail = messages.filter((m) => m.role === "user" || m.role === "assistant").slice(-3);
	const payload = `${messages.length}:${tail.map((m, i) => `[${i}:${m.role ?? ""}]${typeof m.content === "string" ? m.content : JSON.stringify(m.content ?? "")}`).join("\0")}`;
...
async function runMemoryFlushIfNeeded(params) {
	...
	const sessionFilePath = await resolveSessionFilePathForFlush(params.followupRun.run.sessionId, entry ?? params.sessionEntry, params.storePath, params.sessionKey ? resolveAgentIdFromSessionKey(params.sessionKey) : void 0);
	let contextHashBeforeFlush;
	if (sessionFilePath) try {
		const tailMessages = await readTranscriptTailMessages(sessionFilePath, 10);
		const currentPrompt = params.followupRun.prompt;
		if (currentPrompt) tailMessages.push({
			role: "user",
			content: currentPrompt
		});
		if (tailMessages.length === 0) logVerbose(`memoryFlush dedup skipped (no tail messages extracted): sessionKey=${params.sessionKey}`);
		contextHashBeforeFlush = tailMessages.length > 0 ? computeContextHash(tailMessages) : void 0;
		const previousHash = entry?.memoryFlushContextHash;
		if (previousHash && contextHashBeforeFlush === previousHash) {
			logVerbose(`memoryFlush skipped (context hash unchanged): sessionKey=${params.sessionKey} hash=${contextHashBeforeFlush}`);
	...
	try {
		await runWithModelFallback({
			...
			run: async (provider, model, runOptions) => {
				if (contextHashBeforeFlush && fallbackFlushAttemptedForCurrentHash) {
					logVerbose(`memoryFlush fallback candidate skipped (context hash already attempted): sessionKey=${params.sessionKey} hash=${contextHashBeforeFlush} provider=${provider} model=${model}`);
					return {
						payloads: [],
						meta: {}
					};
		...
		if (params.storePath && params.sessionKey) try {
			...
			if (sessionFilePath) try {
				...
				if (postFlushMessages.length > 0) contextHashAfterFlush = computeContextHash(postFlushMessages);
			} catch {}
			const updatedEntry = await updateSessionStoreEntry({
				storePath: params.storePath,
				sessionKey: params.sessionKey,
				update: async () => ({
					...
					memoryFlushContextHash: contextHashAfterFlush ?? void 0
				})
			});
...
*/
async function resolveSessionFilePathForFlush(sessionId, entry, storePath, agentId) {
	if (!sessionId) return;
	return resolveSessionFilePath(sessionId, entry, resolveSessionFilePathOptions({
		agentId,
		storePath
	})) ?? void 0;
}
...
*/
async function readTranscriptTailMessages(filePath, maxMessages) {
	...
	try {
		...
		const lines = (start > 0 ? nlIdx >= 0 ? tail.slice(nlIdx + 1) : "" : tail).split(/\r?\n/);
		const messages = [];
		for (let i = lines.length - 1; i >= 0 && messages.length < maxMessages; i--) {
			...
			try {
				...
				if (parsed?.message?.role) messages.unshift({
...
function loadReplyPayloadsDedupeRuntime() {
	replyPayloadsDedupeRuntimePromise ??= import("./reply-payloads-dedupe.runtime-fiJRF5dE.js");
...
let replyPayloadsRuntimePromise = null;
function loadPiEmbeddedRuntime() {
	piEmbeddedRuntimePromise ??= import("./pi-embedded.runtime-PcrAKFUf.js");
	return piEmbeddedRuntimePromise;
}
function loadRouteReplyRuntime() {
	routeReplyRuntimePromise ??= import("./route-reply.runtime-DL_fhTyP.js");
	return routeReplyRuntimePromise;
}
function loadReplyPayloadsRuntime() {
	replyPayloadsRuntimePromise ??= import("./reply-payloads.runtime-B8Ti4WxZ.js");
	...
	*/
	...
	return async (queued) => {
		try {
			...
			const currentMessageId = queued.messageId?.trim() || void 0;
			const applyFollowupReplyThreading = async (payloads) => {
			...
			const sendCompactionNotice = async (text) => {
				try {
						...
						try {
							...
							const result = await runEmbeddedPiAgent({
								...
								onAgentEvent: (evt) => {
									...
									const phase = typeof evt.data?.phase === "string" ? evt.data.phase : "";
									if (phase === "start") sendCompactionNotice("🧹 Compacting context...");
									const completed = evt.data?.completed === true;
			...
			const lazyContextTokens = agentCfgContextTokens == null && cachedContextTokens == null ? lookupContextTokens(modelUsed, { allowAsyncLoad: false }) : void 0;
			const contextTokensUsed = agentCfgContextTokens ?? cachedContextTokens ?? lazyContextTokens ?? sessionEntry?.contextTokens ?? 2e5;
			...
			const replyTaggedPayloads = await applyFollowupReplyThreading((runResult.payloads ?? []).flatMap((payload) => {
			...
			const { filterMessagingToolDuplicates, filterMessagingToolMediaDuplicates, shouldSuppressMessagingToolReplies } = await loadReplyPayloadsRuntime();
			...
			let finalPayloads = shouldSuppressMessagingToolReplies({
				messageProvider: resolveOriginMessageProvider({
					originatingChannel: queued.originatingChannel,
					provider: queued.run.messageProvider
				}),
			...
			if (autoCompactionCount > 0) {
				...
				finalPayloads = [...await applyFollowupReplyThreading([{
					text: queued.run.verboseLevel && queued.run.verboseLevel !== "off" ? `🧹 Auto-compaction complete${suffix}.` : `✅ Context compacted${suffix}.`,
					replyToCurrent: true,
					isCompactionNotice: true
				}]), ...finalPayloads];
			}
...
let piEmbeddedQueueRuntimePromise = null;
let usageCostRuntimePromise = null;
let sessionStoreRuntimePromise = null;
function loadPiEmbeddedQueueRuntime() {
	piEmbeddedQueueRuntimePromise ??= import("./pi-embedded-queue.runtime-DFYrCysd.js");
	return piEmbeddedQueueRuntimePromise;
}
function loadUsageCostRuntime() {
	usageCostRuntimePromise ??= import("./usage-cost.runtime-B5lyqZkQ.js");
	return usageCostRuntimePromise;
}
function loadSessionStoreRuntime() {
	sessionStoreRuntimePromise ??= import("./store.runtime-BhBkDoLR.js");
...
async function runReplyAgent(params) {
	...
	try {
		...
		const cachedContextTokens = lookupCachedContextTokens(modelUsed);
		const contextTokensUsed = agentCfgContextTokens ?? cachedContextTokens ?? lookupContextTokens(modelUsed, { allowAsyncLoad: false }) ?? activeSessionEntry?.contextTokens ?? 2e5;
		...
		if (autoCompactionCount > 0) {
			...
			const suffix = typeof count === "number" ? ` (count ${count})` : "";
			const completionText = verboseEnabled ? `🧹 Auto-compaction complete${suffix}.` : `✅ Context compacted${suffix}.`;
			if (blockReplyPipeline && opts?.onBlockReply) {
				...
				Promise.race([opts.onBlockReply(noticePayload), new Promise((_, reject) => setTimeout(() => reject(/* @__PURE__ */ new Error("compaction notice timeout")), blockReplyTimeoutMs))]).catch(() => {});
			} else {
				const currentMessageId = sessionCtx.MessageSidFull ?? sessionCtx.MessageSid;
				verboseNotices.push(applyReplyToMode({
					text: completionText,
					replyToId: currentMessageId,
					replyToCurrent: true,
					isCompactionNotice: true
