...
import { $n as splitShellArgs, Rt as loadConfig, un as parseDurationMs } from "./model-selection-UzsaUG4U.js";
...
import { f as listMemoryFiles, p as normalizeExtraMemoryPaths } from "./query-expansion-Cx7sh0DN.js";
...
async function getMemorySearchManager(params) {
	...
	if (resolved.backend === "qmd" && resolved.qmd) {
		...
		try {
			const { QmdMemoryManager } = await import("./qmd-manager-xI0Sy6GH.js");
			...
			if (primary) {
				...
				const wrapper = new FallbackMemoryManager({
					...
					fallbackFactory: async () => {
						const { MemoryIndexManager } = await import("./manager-AHTboDq7.js").then((n) => n.t);
