...
import { Kt as splitShellArgs, j as loadConfig, rt as parseDurationMs } from "./auth-profiles-CKfLk8eI.js";
...
import { f as listMemoryFiles, p as normalizeExtraMemoryPaths } from "./query-expansion-6M4HIXKJ.js";
...
async function getMemorySearchManager(params) {
	...
	if (resolved.backend === "qmd" && resolved.qmd) {
		...
		try {
			const { QmdMemoryManager } = await import("./qmd-manager-BZKoUWjg.js");
			...
			if (primary) {
				...
				const wrapper = new FallbackMemoryManager({
					...
					fallbackFactory: async () => {
						const { MemoryIndexManager } = await import("./manager-BWJEsXOv.js").then((n) => n.t);
